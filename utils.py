...
def ComputeCIndex( affinityScores, predictedScores ):
    cnt = 0
    cIndex = 0

    for i in tqdm.tqdm( range(len(affinityScores)) ):
        for j in range(len(affinityScores)):
            if i != j and affinityScores[i] > affinityScores[j]:
                cnt += 1
                cIndex += float(((predictedScores[i] > predictedScores[j]) + \
                    (predictedScores[i] == predictedScores[j]) * 0.5).item())
